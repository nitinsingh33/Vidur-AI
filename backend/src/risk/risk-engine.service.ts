...
export class RiskEngineService {
  assess(input: RiskAssessmentInput): RiskAssessment {
    ...
    return {
      ...
      expectedLoss,
      /*
       * The real money currently exposed — the full payment/order/
       * invoice amount, not a probability-discounted figure. This is
       * what merchants see as "revenue at risk," and it must match
       * what "revenue recovered" is measured in (the full amount),
       * or the recovery-rate math (recovered / (atRisk + recovered))
       * doesn't mean anything. expectedLoss (the discounted figure)
       * is still used for risk-tier classification below — it's a
       * reasonable prioritization signal, just not what "at risk"
       * should say to a merchant.
       */
      revenueAtRisk: input.amount,
